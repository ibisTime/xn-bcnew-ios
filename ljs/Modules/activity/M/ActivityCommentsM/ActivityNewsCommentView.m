...
@interface ActivityNewsCommentView()<UITableViewDataSource, UITableViewDelegate>

@end

@implementation ActivityNewsCommentView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //分区1热门评论 分区2最新评论
//    InfoCommentModel *commentModel = indexPath.section == 0 ? self.detailModel.hotCommentList[indexPath.row]: self.newestComments[indexPath.row];
    if (self.newestComments.count > 0) {
        commentModel = self.newestComments[indexPath.row];
    ...
    if (cell == nil) {
        cell = [[InfoCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:infoCommentCellID];
    }
