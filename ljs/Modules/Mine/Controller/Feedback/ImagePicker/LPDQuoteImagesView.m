...
//    [cell.imageThumbnail theme_setBackgroundColorIdentifier:BackColor moduleName:ColorName];
    if(_selectedPhotos.count<_maxSelectedCount) {
        ...
        if (indexPath.row == _selectedPhotos.count) {
            cell.imageThumbnail.contentMode = UIViewContentModeScaleAspectFit;
            [cell.imageThumbnail setImage:kImage(@"添加自选")];
...
- (void)imagePickerController:(LPDImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
  ...
  NSLog(@"%f",self.collectionView.collectionViewLayout.collectionViewContentSize.width);
  [_collectionView reloadData];
